...
/datum/antagonist/bloodsucker/proc/give_warning(atom/source, danger_level, vampire_warning_message, vassal_warning_message)
	...
	switch(danger_level)
		if(DANGER_LEVEL_FIRST_WARNING)
			owner.current.playsound_local(null, 'surfshack13/sound/bloodsuckers/griffin_3.ogg', vol = 50, vary = TRUE)
		if(DANGER_LEVEL_SECOND_WARNING)
			owner.current.playsound_local(null, 'surfshack13/sound/bloodsuckers/griffin_5.ogg', vol = 50, vary = TRUE)
...
/atom/movable/screen/alert/status_effect/bloodsucker_sol
	...
	icon = 'surfshack13/icons/bloodsuckers/actions_bloodsucker.dmi'
