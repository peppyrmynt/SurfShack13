...
/obj/structure/slime_crystal/purple/on_mob_effect(mob/living/affected_mob)
	...
	switch(rand_dam_type)
		if(1)
			if(carbon_mob.getBruteLoss() > 0)
				need_mob_update += carbon_mob.adjustBruteLoss(-heal_amt, updating_health = FALSE)
				healed_last_tick = TRUE
		if(2)
			if(carbon_mob.getFireLoss() > 0)
				need_mob_update += carbon_mob.adjustFireLoss(-heal_amt, updating_health = FALSE)
				healed_last_tick = TRUE
		if(3)
			if(carbon_mob.getOxyLoss() > 0)
				need_mob_update += carbon_mob.adjustOxyLoss(-heal_amt, updating_health = FALSE)
				healed_last_tick = TRUE
		if(4)
			if(carbon_mob.getToxLoss() > 0)
				need_mob_update += carbon_mob.adjustToxLoss(-heal_amt, updating_health = FALSE, forced = TRUE) //don't kill jellypeople
				healed_last_tick = TRUE
	...
	for(var/obj/effect/decal/cleanable/B in range(effect_range,src))
		if(istype(B, /obj/effect/decal/cleanable/blood))
			...
			if(blood_amt > max_blood_amt)
				...
				break
			qdel(B)
		else if (istype(B, /obj/effect/decal/cleanable/blood/footprints))
			blood_amt += 3
			if(blood_amt > max_blood_amt)
				blood_amt = max_blood_amt
				break
...
/obj/structure/slime_crystal/gold/attack_hand(mob/user)
	...
	if(chosen_pet)
		...
		human_mob.add_traits(list(TRAIT_GODMODE, TRAIT_MUTE), "gold_crystal")
...
/obj/structure/slime_crystal/gold/on_mob_leave(mob/living/affected_mob)
	...
	if(!human_mob)
		return
	human_mob.remove_traits(list(TRAIT_GODMODE, TRAIT_MUTE), "gold_crystal")
