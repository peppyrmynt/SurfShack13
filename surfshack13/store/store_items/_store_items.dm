...
/datum/store_item/proc/attempt_purchase(client/buyer)
	...
	if(!one_time_buy)
		if(finalize_purchase(buyer))
			buyers_preferences.adjust_metacoins(buyer.ckey, -item_cost, "[buyer] bought a [name] for [item_cost]")
		return
	else
		buyers_preferences.adjust_metacoins(buyer.ckey, -item_cost, "[buyer] bought a [name] for [item_cost]")
	attempt_spawn(buyer)
...
/datum/store_item/proc/attempt_spawn(client/buyer)
	...
	if(!isliving(buyer_mob))
		buyer.prefs.adjust_metacoins(buyer.ckey, item_cost, "Spawned as Non-Living, Unable to utilize items")
		return
