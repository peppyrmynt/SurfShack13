...
/mob/living/silicon/robot/proc/self_destruct(mob/demoman)
	var/turf/groundzero = get_turf(src)
	message_admins(span_notice("[ADMIN_LOOKUPFLW(demoman)] detonated [key_name_admin(src, client)] at [ADMIN_VERBOSEJMP(groundzero)]!"))
	demoman.log_message("detonated [key_name(src)]!", LOG_ATTACK)
	log_message("was detonated by [key_name(demoman)]!", LOG_ATTACK, log_globally = FALSE)

	log_combat(demoman, src, "detonated cyborg")
	log_silicon("CYBORG: [key_name(src)] has been detonated by [key_name(demoman)].")
	if(connected_ai)
