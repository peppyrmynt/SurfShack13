...
/datum/dna/proc/generate_unique_features()
	...
	if(features["tail_saiyan"])
		L[DNA_SAIYAN_TAIL_BLOCK] = construct_block(GLOB.tails_list_human.Find(features["tail_saiyan"]), GLOB.tails_list_human.len)
	if(features["snout"])
...
/datum/dna/proc/update_uf_block(blocknumber)
	...
	switch(blocknumber)
		...
		if(DNA_SAIYAN_TAIL_BLOCK)
			set_uni_feature_block(blocknumber, construct_block(GLOB.tails_list_human.Find(features["tail_saiyan"]), GLOB.tails_list_saiyan.len))
		if(DNA_SNOUT_BLOCK)
...
/mob/living/carbon/human/updateappearance(icon_update = TRUE, mutcolor_update = FALSE, mutations_overlay_update = FALSE)
	...
	if(dna.features["tail_saiyan"])
		dna.features["tail_saiyan"] = GLOB.tails_list_lizard[deconstruct_block(get_uni_feature_block(features, DNA_SAIYAN_TAIL_BLOCK), GLOB.tails_list_saiyan.len)]
