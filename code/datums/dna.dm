...
/datum/dna/proc/generate_unique_features()
	...
	if(features["tail_saiyan"])
		L[DNA_SAIYAN_TAIL_BLOCK] = construct_block(SSaccessories.tails_list_saiyan.Find(features["tail_saiyan"]), length(SSaccessories.tails_list_saiyan))
	if(features["snout"])
...
/datum/dna/proc/update_uf_block(blocknumber)
	...
	switch(blocknumber)
		...
		if(DNA_SAIYAN_TAIL_BLOCK)
			set_uni_feature_block(blocknumber, construct_block(SSaccessories.tails_list_saiyan.Find(features["tail_saiyan"]), length(SSaccessories.tails_list_saiyan)))
		if(DNA_SNOUT_BLOCK)
...
/mob/living/carbon/human/updateappearance(icon_update = TRUE, mutcolor_update = FALSE, mutations_overlay_update = FALSE)
	...
	if(dna.features["tail_saiyan"])
		dna.features["tail_saiyan"] = SSaccessories.tails_list_saiyan[deconstruct_block(get_uni_feature_block(features, DNA_SAIYAN_TAIL_BLOCK), length(SSaccessories.tails_list_saiyan))]
...
/mob/living/carbon/proc/easy_random_mutate(quality = POSITIVE + NEGATIVE + MINOR_NEGATIVE, scrambled = TRUE, sequence = TRUE, exclude_monkey = TRUE, resilient = NONE, return_mut_type = FALSE)
	if(!has_dna())
		CRASH("[src] does not have DNA")
	var/list/mutations = list()
	if(quality & POSITIVE)
